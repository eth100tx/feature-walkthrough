...
Treat narration as part of the first finished pitch. During briefing, check the requested language, audience, runtime, available providers and credentials, and whether this environment can actually listen to audio. After story agreement, generate a short representative sample before the full track or final animation timing. Do this as an internal quality check; do not add a mandatory voice-selection question when a suitable default and authorization already exist.

Prefer a capable conversational neural voice or a suitable human recording. Choose by the actual delivery, not the first voice in a list, a familiar legacy name, or the word “neural.” Use an available direction-capable service when it fits the task and authorized access exists. Basic operating-system speech and browser `speechSynthesis` are useful timing drafts, not the default voice for a professional pitch. Do not silently substitute them when a production voice is unavailable.

For an English pitch using an available Microsoft Edge speech path, **Ava Multilingual Neural** is a useful starting candidate; compare Andrew or Emma when available and appropriate. In the TA Studio iteration, the user accepted Ava at -3% after an Aria first pass, alongside a rewritten script. That is a project preference, not a controlled finding that one voice is universally better. Recheck current voice availability, language fit and supported controls. Do not force this provider or voice when the user selected another, a better suited authorized path exists, or the voice is unavailable.

If a suitable production path is unavailable, complete the script and independent visual work, state the specific missing capability, and label any timing audio as a draft. Ask only for the missing access or choice needed to proceed; do not request secrets in chat or imply that paid service access is included.
...
Open with recognition of the viewer's job and the result they want. Keep implementation caveats and exception details proportional to the story. Preserve necessary simulation disclosure in the spoken introduction, but weave it into the setup rather than making a disclaimer the value statement. Cut procedural detail before increasing speaking speed to hit a runtime.
...
Choose a 15–25 second passage containing the value statement, a role handoff or meaningful change, a product name or difficult term, and an outcome. Generate two plausible voice/delivery candidates with identical words when feasible. For a voice/rate-only engine, compare supported voices or settings rather than inventing style controls. An already accepted voice can remain the default, but spot-check it with the new script. These durations/styles are production starting points, not research-backed thresholds.
...
If listening is unavailable, say so before representing the audio as reviewed. Use a previously user-accepted voice/settings when applicable, or the best supported candidate with a concise rationale. Generate the sample and continue authorized production rather than blocking unrelated work. Provide a playable sample or finished track for human listening, and mark **listening review pending**. Never equate “samples generated,” a successful decode, a waveform check, or `muted: false` with hearing the delivery. Distinguish a usable first version from an audio-quality sign-off; do not claim the latter without a listening review. Using a neural voice alone does not pass the professional-audio quality check, and pending listening is not a pass.
...
- Keep a manifest of voice/settings, segment identity, measured speech boundaries and total duration. Derive captions, transcript and displayed runtime from that record. Check that every scene/phase has its intended audio and that captions reflect the final spoken take.
- Decode the final export, check level consistency and clipping, and listen to the full track and segment joins when possible. Preserve source quality and avoid repeated lossy re-encoding; a larger file or higher nominal bitrate cannot repair a weak source performance. Keep background music optional and subordinate to speech.
...
Record which provider/voice/settings were used, why they were chosen, what was actually heard, what objective playback checks passed, and what remains unverified. Keep this short; it is evidence of the production decision, not a new approval ceremony.
